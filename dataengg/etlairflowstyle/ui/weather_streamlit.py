...
def fetch_data(query):
    conn = st.connection("sql", type="sql", url="mysql+pymysql://root:password@localhost:3306/etl_db")
...
last_updated_query = "SELECT DATE_FORMAT(FROM_UNIXTIME(MAX(last_updated_epoch)), '%Y-%m-%d %H:%i:%s') AS last_updated_ts FROM weather;"
...
SELECT name, lat, lon, temp_c, temp_f, condition_text, condition_icon, humidity, cloud
...
for index, row in weather_df.iterrows():
    location = int(row['lat']), int(row['lon'])
    html_tooltip = HTML_TOOLTIP.format(name=row['name'], temperature=round(row['temp_c']), \
        condition=row['condition_text'], image_url='https:' + row['condition_icon'])
    folium.Marker(location, tooltip=html_tooltip).add_to(map)

st_map = st_folium(map, width=700, height=450)

presented_table = weather_df[['name', 'lat', 'lon', 'temp_c', 'temp_f', 'condition_text']]
st.dataframe(
    presented_table,
    column_config={
        "name": "City",
