...
# Initialize model
'''
Multinomial (Softmax): Trains a single model that uses the Softmax function to predict 
                       a probability distribution across all classes. This is generally 
                       more accurate for mutually exclusive classes.
One-vs-Rest (OvR): Also known as "One-vs-All," this method trains one binary classifier 
                   per class (e.g., "Class A vs. Not Class A"). The final prediction is 
                   based on the classifier with the highest confidence score.
'''
# multi_class options: 'multinomial', 'ovr', or 'auto' (default)
